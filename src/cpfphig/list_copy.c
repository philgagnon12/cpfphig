                                 ...
                                 CPFPHIG_OPTIONAL struct cpfphig_error*    Error )
{
    ...
    {
        if( Error != NULL )
            cpfphig_error_message(cpfphig_system_error, "List_Copy is NULL", Error );
    ...
    next_ret                = CPFPHIG_OK;
    next_error.error_type   = cpfphig_ok;

    while( ret == CPFPHIG_OK &&
           CPFPHIG_OK == ( next_ret = cpfphig_list_next( &list_iterator,
                                                         &item,
                                                         &next_error ) ) )
    {
    ...
    if( next_error.error_type   == cpfphig_system_error &&
        next_ret                == CPFPHIG_FAIL )
    {
        if( Error != NULL )
            *Error = next_error;
